...
export default function AtivacaoUsuariosPage() {
    ...
    const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>(() => {
        try {
            const raw = localStorage.getItem('ativacao_usuarios_visible_columns')
            return raw ? JSON.parse(raw) : { usuario: true, nivel: true, status: true }
        } catch (e) {
            return { usuario: true, nivel: true, status: true }
        }
    })
    ...
    useEffect(() => {
        try {
            localStorage.setItem('ativacao_usuarios_visible_columns', JSON.stringify(visibleColumns))
        } catch (e) { }
    }, [visibleColumns])
    ...
    return (
        <div className="p-2 md:p-4 lg:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Ativação de Usuários</h1>
